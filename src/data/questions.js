...
{n:"B1",text:"Use the binomial series to expand 1/√(1+x) = (1+x)^(−1/2) as a power series. Write out the first four terms and give the general term. Converges for |x|<1.",ptype:"binom"},
{n:"B2",text:"Use the binomial series to expand √(1+x) = (1+x)^(1/2) as a power series. Write out the first four terms. Converges for |x|<1.",ptype:"binom"},
{n:"B3",text:"Use the binomial series to expand 1/(1+x)² = (1+x)^(−2). Write the general term C(−2,n)·xⁿ. Simplify: C(−2,n) = (−1)ⁿ(n+1). Converges for |x|<1.",ptype:"binom"},
{n:"B4",text:"Use the binomial series to expand (1−x²)^(1/2). Substitute −x² for x in the √(1+x) series. State the interval of convergence.",ptype:"binom"},
{n:"B5",text:"Use the binomial series to expand 1/√(4+x). First factor: 1/√(4+x) = (1/2)·(1+x/4)^(−1/2). Then apply the binomial series with x replaced by x/4. State the interval of convergence.",ptype:"binom"}
]},
...
{n:"p683-1",text:"Use the Lagrange Remainder to bound the error when approximating e by the nth Taylor polynomial of eˣ at x=1. |Rₙ(1)| ≤ M·1^(n+1)/(n+1)! where M=e<3. Find the smallest n so that 3/(n+1)! < 0.001.",ptype:"lagrange"},
{n:"p683-2",text:"How many terms of the Maclaurin series for sinx are needed to approximate sin(0.5) with error less than 0.0001? Use |Rₙ| ≤ (0.5)^(n+1)/(n+1)!. Try successive values of n until bound < 0.0001.",ptype:"lagrange"},
{n:"p683-3",text:"Approximate cos(0.2) using a Taylor polynomial and bound the error using the Lagrange Remainder. |cos⁽ⁿ⁺¹⁾(t)| ≤ 1 for all t, so |Rₙ| ≤ (0.2)^(n+1)/(n+1)!. Find n so error < 0.00001.",ptype:"lagrange"},
{n:"p684-1",text:"The Lagrange form of the remainder for a Taylor polynomial of degree n centred at a is Rₙ(x) = f⁽ⁿ⁺¹⁾(c)·(x−a)^(n+1)/(n+1)! for some c between a and x. Use this to show that the Maclaurin series for eˣ converges to eˣ for all x by showing |Rₙ(x)|→0.",ptype:"lagrange"},
{n:"p684-2",text:"Show that the Maclaurin series for sinx converges to sinx for all x. |Rₙ(x)| ≤ |x|^(n+1)/(n+1)! → 0 for all x since |x|^(n+1)/(n+1)! is a term of the convergent series for e^|x|.",ptype:"lagrange"},
{n:"p685-1",text:"Approximate √(1.1) using a 2nd degree Taylor polynomial centred at x=1 for f(x)=√x. Then use the Lagrange Remainder to bound the error. f'''(x)=3/(8x^(5/2)); on [1,1.1], |f'''(c)| ≤ 3/8. |R₂| ≤ (3/8)·(0.1)³/6.",ptype:"lagrange"},
{n:"p685-2",text:"Use the Taylor polynomial of degree 4 for cosx to approximate cos(π/3) and bound the error. Compare with exact value. Show |R₄| ≤ (π/3)⁵/5! ≈ 0.0004.",ptype:"lagrange"}
]}
...
{n:"1",text:"Verify y₁=e^(2x), y₂=e^(−3x) satisfy y''+y'−6y=0. Confirm they are linearly independent. Write the general solution by superposition.",ptype:"superpos",solution:"Verify $y₁$: $4e²ˣ+2e²ˣ−6e²ˣ = 0$ ✓\nVerify $y₂$: $9e⁻³ˣ−3e⁻³ˣ−6e⁻³ˣ = 0$ ✓\nLinearly independent (different exponentials).\n>>$y = C₁e²ˣ+C₂e⁻³ˣ$<<"},
...
{n:"4",text:"Verify y₁=3e^(4x) and y₂=−2xe^(4x) satisfy 2y''−16y'+32y=0. Confirm they are linearly independent. Write the general solution.",ptype:"superpos",solution:"Verify $y₁=3e⁴ˣ$: $2(48)−16(12)+32(3) = 96−192+96 = 0$ ✓\nVerify $y₂=−2xe⁴ˣ$: $y₂'=−2e⁴ˣ−8xe⁴ˣ$, $y₂''=−16e⁴ˣ−32xe⁴ˣ → (−32−64x+32+128x−64x)e⁴ˣ = 0$ ✓\nIndependent ($y₂$ has $x$ factor).\n>>$y = (C₁+C₂x)e⁴ˣ$<<"},
{n:"5",text:"Verify y₁=10, y₂=3eˣ, y₃=4e^(2x) satisfy y'''−3y''+2y'=0. Confirm they are linearly independent. Write the general solution.",ptype:"superpos",solution:"Verify $y₁=10$: $0−0+0 = 0$ ✓\nVerify $y₂=3eˣ$: $(3−9+6)eˣ = 0$ ✓\nVerify $y₃=4e²ˣ$: $32−48+16 = 0$ ✓\nAll independent (constant, $eˣ$, $e²ˣ$).\n>>$y = C₁+C₂eˣ+C₃e²ˣ$<<"},
{n:"6",text:"Verify y₁=e^(2x)cos(x/√2), y₂=e^(2x)sin(x/√2) satisfy 2y''−8y'+9y=0. Confirm they are linearly independent. Write the general solution.",ptype:"superpos",solution:"Auxiliary: $2m²−8m+9=0 → m=(8±√(64−72))/4=2±(√2/2)i$.\nVerify $y₁$, $y₂$ by substitution: both give $0$ ✓\nIndependent (cos vs. sin factor).\n>>$y = e²ˣ(C₁cos(x/√2)+C₂sin(x/√2))$<<"},
...
{n:"15",text:"Use the Wronskian to test linear independence: {x sinx, e^(2x)} on −∞ < x < ∞.",ptype:"reduct",solution:"$W = |x sinx, e²ˣ; sinx+x cosx, 2e²ˣ| = e²ˣ(2x sinx−sinx−x cosx)$.\nAt $x=π$: $W = e²π(0−0+π) = πe²π ≠ 0$.\n>>LINEARLY INDEPENDENT<<"}
]},
