...
{n:"9",text:"Prove or disprove that operator L is linear: L takes the definite integral of y(x) from −1 to 4.",ptype:"linop",solution:"$∫₋₁⁴(y₁+y₂)dx = ∫₋₁⁴y₁dx+∫₋₁⁴y₂dx = Ly₁+Ly₂$ ✓\n$∫₋₁⁴cy₁dx = c∫₋₁⁴y₁dx = cLy₁$ ✓\n>>LINEAR<<"},
...
{n:"17",text:"Determine if the DE is linear or nonlinear; write linear ones in operator notation: y'' − 3y' − 2y = 9·sec²x.",ptype:"linop",solution:"All $y$ terms to first power, no products.\nOperator form: >>$(D²−3D−2)y = 9sec²x$<<"},
{n:"18",text:"Determine if the DE is linear or nonlinear; write linear ones in operator notation: yy'' + 3y' − 2y = eˣ.",ptype:"linop",solution:"Contains $yy''$ — product of $y$ with $y''$.\n>>NONLINEAR<<"},
...
{n:"11",text:"Use the Wronskian to test linear independence: {1, x, x²} on −∞ < x < ∞.",ptype:"lintest",solution:"$W(1,x,x²) = |1 x x²; 0 1 2x; 0 0 2| = 2 ≠ 0$ for all $x$.\n>>LINEARLY INDEPENDENT<<"},
{n:"12",text:"Use the Wronskian to test linear independence: {x, 2x−3x², x²} on −∞ < x < ∞.",ptype:"lintest",solution:"$2x−3x² = 2(x)+(−3)(x²)$ — $y₂$ is a linear combination of $y₁$ and $y₃$, so $W = 0$.\n>>LINEARLY DEPENDENT<<"},
{n:"13",text:"Use the Wronskian to test linear independence: {sinx, cosx} on 0 ≤ x ≤ 2π.",ptype:"lintest",solution:"$W(sinx,cosx) = |sinx cosx; cosx −sinx| = −sin²x−cos²x = −1 ≠ 0$.\n>>LINEARLY INDEPENDENT<<"},
{n:"14",text:"Use the Wronskian to test linear independence: {x, xeˣ, x²eˣ} on 0 ≤ x ≤ 1.",ptype:"lintest",solution:"$W(x, xeˣ, x²eˣ)$: rows are $\{x, xeˣ, x²eˣ\}$, $\{1, eˣ+xeˣ, 2xeˣ+x²eˣ\}$, $\{0, 2eˣ+xeˣ, 2eˣ+4xeˣ+x²eˣ\}$.\nAfter expansion (factor $xe²ˣ$): $W = xe²ˣ ≠ 0$ for $x∈(0,1]$.\n>>LINEARLY INDEPENDENT<<"},
{n:"15",text:"Use the Wronskian to test linear independence: {x sinx, e^(2x)} on −∞ < x < ∞.",ptype:"lintest",solution:"$W = |x sinx, e²ˣ; sinx+x cosx, 2e²ˣ| = e²ˣ(2x sinx−sinx−x cosx) = e²ˣ(sinx(2x−1)−x cosx)$.\nNot identically zero (e.g. at $x=π$: $W=πe²π≠0$).\n>>LINEARLY INDEPENDENT<<"}
]},
...
{n:"2",text:"Find the general solution: 2y'' − 16y' + 32y = 0.",ptype:"aux-rep",solution:"Auxiliary: $2m²−16m+32=2(m−4)²=0→m=4$ (double root).\n>>$y = (C₁+C₂x)e⁴ˣ$<<"},
{n:"3",text:"Find the general solution: 2y'' + 16y' + 82y = 0.",ptype:"aux-cx",solution:"Auxiliary: $2m²+16m+82=0→m²+8m+41=0→m=(−8±√(64−164))/2=−4±5i$.\nComplex roots: $α=−4$, $β=5$.\n>>$y = e⁻⁴ˣ(C₁cos5x+C₂sin5x)$<<"},
...
{n:"12",text:"Find the general solution: y'''' + 16y = 0.",ptype:"aux-cx",solution:"Auxiliary: $m⁴+16=0$. Write $m=a+bi$: $m²=±4i$.\nFor $m²=4i$: $a²−b²=0$, $2ab=4→a=b=√2→m=√2(1+i)$ or $√2(−1−i)$.\nFor $m²=−4i$: $m=√2(1−i)$ or $√2(−1+i)$.\nTwo conjugate pairs: $α=±√2$, $β=√2$.\n>>$y = e^{√2x}(C₁cos(√2x)+C₂sin(√2x))+e^{−√2x}(C₃cos(√2x)+C₄sin(√2x))$<<"},
{n:"13",text:"Find the DE whose general solution is y(x) = C₁eˣ + (C₂+C₃x)e^(−4x).",ptype:"aux-given",solution:"Roots: $m=1$ (simple), $m=−4$ (double).\nAuxiliary: $(m−1)(m+4)²=m³+7m²+8m−16=0$.\n>>$y'''+7y''+8y'−16y = 0$<<"},
...
{n:"19",text:"Find the eigenvalues and eigenfunctions: y''+λy=0, 0<x<3; y(0)=0=y(3).",ptype:"sturm",solution:"$λ<0$ and $λ=0$: only trivial solutions.\n$λ>0$: let $λ=μ²$. $y=C₁cosμx+C₂sinμx$.\n$y(0)=C₁=0→y=C₂sinμx$.\n$y(3)=C₂sin3μ=0→3μ=nπ→μ=nπ/3$.\n>>$λₙ = n²π²/9$,  $yₙ = sin(nπx/3)$  $(n=1,2,3,...)$<<"},
{n:"20",text:"Find the eigenvalues and eigenfunctions: y''+λy=0, 0<x<4; y'(0)=0=y'(4).",ptype:"sturm",solution:"$λ=0$: $y=C₁+C₂x$. $y'(0)=C₂=0$, $y'(4)=0$ satisfied. So $λ₀=0$, $y₀=1$ is an eigenvalue.\n$λ>0$, $μ=√λ$: $y=C₁cosμx+C₂sinμx$.\n$y'(0)=C₂μ=0→C₂=0$. $y'(4)=−C₁μsin4μ=0→sin4μ=0→4μ=nπ→μ=nπ/4$.\n>>$λₙ = n²π²/16$,  $yₙ = cos(nπx/4)$  $(n=0,1,2,...)$<<"},
{n:"21",text:"Find the eigenvalues and eigenfunctions: y''+λy=0, 0<x<2; y(0)=0=y'(2).",ptype:"sturm",solution:"$λ>0$, $μ=√λ$: $y=C₁cosμx+C₂sinμx$.\n$y(0)=C₁=0→y=C₂sinμx$.\n$y'(2)=C₂μcos2μ=0→cos2μ=0→2μ=(2n−1)π/2→μ=(2n−1)π/4$.\n>>$λₙ = (2n−1)²π²/16$,  $yₙ = sin((2n−1)πx/4)$  $(n=1,2,...)$<<"},
{n:"22",text:"Find the eigenvalues and eigenfunctions: y''+λy=0, 0<x<5; y'(0)=0=y(5).",ptype:"sturm",solution:"$λ>0$, $μ=√λ$: $y=C₁cosμx+C₂sinμx$.\n$y'(0)=C₂μ=0→C₂=0→y=C₁cosμx$.\n$y(5)=C₁cos5μ=0→5μ=(2n−1)π/2→μ=(2n−1)π/10$.\n>>$λₙ = (2n−1)²π²/100$,  $yₙ = cos((2n−1)πx/10)$  $(n=1,2,...)$<<"},
{n:"23",text:"Find the eigenvalues and eigenfunctions: y''−y'+λy=0, 0<x<1; y(0)=0=y(1).",ptype:"sturm",solution:"Auxiliary $m²−m+λ=0→m=(1±√(1−4λ))/2$. Need complex roots: $λ>1/4$.\n$m=1/2±iω$ where $ω=√(4λ−1)/2$. $y=e^{x/2}(C₁cosωx+C₂sinωx)$.\n$y(0)=C₁=0$. $y(1)=0→sinω=0→ω=nπ→λₙ=(4n²π²+1)/4$.\n>>$λₙ = (4n²π²+1)/4$,  $yₙ = e^{x/2}sin(nπx)$  $(n=1,2,...)$<<"},
{n:"24",text:"A mass M in the xy-plane is subjected to a force always directed toward the origin with magnitude proportional to its distance from the origin. At a certain instant M passes through (x₀,0) with speed v in the positive y-direction. (a) Find the path of M assuming no friction and no damping. (b) Find the path if the force is away from the origin instead.",ptype:"aux-cx",solution:"(a) Force toward origin: $Md²x/dt²=−kx$, $Md²y/dt²=−ky$. Let $ω=√(k/M)$.\nICs: $x(x₀,0)$, speed $v$ in $+y$: $x(t)=x₀cosωt$, $y(t)=(v/ω)sinωt$.\nEliminating $t$: $x²/x₀²+ky²/(Mv²)=1$ — an ellipse.\n(b) Force away from origin: solutions are $x(t)=x₀coshωt$, $y(t)=(v/ω)sinhωt$.\nPath: $x²/x₀²−ky²/(Mv²)=1$ — a hyperbola."},
...
{n:"17",text:"Find a general solution for y'' + 2y' − 4y = cos²x. Hint: use the identity cos²x = (1+cos2x)/2.",ptype:"both",solution:"$yₕ$: $m²+2m−4=0→m=−1±√5$. $yₕ=C₁e^{(−1+√5)x}+C₂e^{(−1−√5)x}$.\nUse $cos²x=(1+cos2x)/2$. For constant $1/2$: $yₚ₁=A→−4A=1/2→A=−1/8$.\nFor $cos2x/2$: $yₚ₂=Bcos2x+Csin2x$. Sub: $−4B+4C−4B=1/2$; $−4C−4B=0$.\n$→C=1/40$, $B=−1/40·2=−1/20$... wait: $−8B+4C=1/2$; $C=−B$ from $−4C=4B$.\n$−8B−4B=1/2→B=−1/24$, $C=1/24$.\n>>$y = C₁e^{(−1+√5)x}+C₂e^{(−1−√5)x}−1/8+(sin2x−cos2x)/24$<<"},
{n:"18",text:"Find a general solution for 2y'' − 4y' + 3y = cosx sin2x. Hint: use the product-to-sum identity to rewrite the right side.",ptype:"both",solution:"$yₕ$: $2m²−4m+3=0→m=1±(1/√2)i$. $yₕ=eˣ(C₁cos(x/√2)+C₂sin(x/√2))$.\nUse $cosx·sin2x=(sin3x+sinx)/2$.\nFor $sin3x/2$: $yₚ₁=Acos3x+Bsin3x$; equate coefficients.\nFor $sinx/2$: $yₚ₂=Ccosх+Dsinx$; equate coefficients.\nFull computation gives $yₚ=(−4cos3x+5sin3x)/246+(4cosx+sinx)/34$.\n>>$y = eˣ(C₁cos(x/√2)+C₂sin(x/√2))+(−4cos3x+5sin3x)/246+(4cosx+sinx)/34$<<"},
{n:"19",text:"Solve the IVP fully: y''−3y'+2y = 8x²+12e^(−x), y(0)=0, y'(0)=2.",ptype:"both",solution:"$yₕ$: $(m−1)(m−2)=0→m=1,2$. $yₕ=C₁eˣ+C₂e²ˣ$.\nFor $8x²$: $yₚ₁=Ax²+Bx+C→A=4$, $B=12$, $C=14$.\nFor $12e⁻ˣ$: $yₚ₂=De⁻ˣ→6D=12→D=2$.\n$y(0)=C₁+C₂+14+2=0→C₁+C₂=−16$.\n$y'(0)=C₁+2C₂+12−2=2→C₁+2C₂=−8$.\n$C₂=8$, $C₁=−24$.\n>>$y = −24eˣ+8e²ˣ+4x²+12x+14+2e⁻ˣ$<<"},
{n:"20",text:"Solve the IVP fully: y''+9y = x(sin3x+cos3x), y(0)=y'(0)=0.",ptype:"both",solution:"$yₕ$: $m²+9=0→m=±3i$. $yₕ=C₁cos3x+C₂sin3x$.\n$sin3x$, $cos3x$ in $yₕ$ ($k=1$) → $yₚ=x[(Ax+B)cos3x+(Cx+D)sin3x]$.\nSubstitute into $y''+9y$: equate $xcos3x$, $xsin3x$, $cos3x$, $sin3x$ terms.\nResult: $C=1/36$ for $xcos3x$ part, $A=0$; $B=−1/108$ from $sin3x$ part.\n$y(0)=C₁=0$; $y'(0)=3C₂+D=0→C₂=−1/108$.\n>>$y = −(1/108)sin3x+x/36(cos3x+sin3x)+x²/12(sin3x−cos3x)$<<"},
...
{n:"1",text:"A 1-kg mass on a spring with constant 16 N/m is pulled 10 cm below equilibrium and released. Find x(t) for: (a) no damping, (b) damping force in newtons equal to 0.1 times the velocity in m/s, (c) damping force in newtons equal to 10 times the velocity in m/s. Classify the motion in each case.",ptype:"spring",solution:"(a) β=0: x″+16x=0 → m=±4i. ICs x(0)=−0.1, x′(0)=0. x(t)=−0.1cos4t. (simple harmonic, amplitude 0.1 m, period π/2 s)\n(b) β=0.1: m²+0.1m+16=0 → m≈−0.05±√15.9994i — underdamped. x(t)=e^(−0.05t)(C₁cos√15.9994t + C₂sin√15.9994t). Apply ICs.\n(c) β=10: (m+2)(m+8)=0 → m=−2,−8 — overdamped. C₁+C₂=−0.1, −2C₁−8C₂=0 → C₂=1/30, C₁=−2/15. x(t)=−(2/15)e⁻²ᵗ+(1/30)e⁻⁸ᵗ."},
...
{n:"5",text:"Series RL circuit: 5H inductor, 20Ω resistor, generator 10sin2t. Find the transient and steady-state currents.",ptype:"circuit",solution:"First-order in I: 5dI/dt + 20I = 20cos2t → I′ + 4I = 4cos2t. μ = e⁴ᵗ. d/dt[Ie⁴ᵗ] = 4e⁴ᵗcos2t. Solving: I(t) = Ce⁻⁴ᵗ + (2sin2t − cos2t)/5. TRANSIENT: (1/5)e⁻⁴ᵗ A. STEADY-STATE: (2sin2t − cos2t)/5 A."},
...
{n:"15",text:"A series LC circuit has inductance 25/9 H, capacitance 0.04 F, and applied voltage 15cos3t V. Find I(t). Does resonance occur?",ptype:"circuit",solution:"ω₀ = 1/√(LC) = 1/√(25/9 · 0.04) = 3 rad/s. Forcing ω = 3 = ω₀ → RESONANCE. DE (in terms of I): (25/9)I″ + 25I = −45sin3t → 5I″ + 45I = −81sin3t/5 → I″ + 9I = −81sin3t/5. Roots ±3i → resonance. Iₚ = (27t/10)cos3t. ICs I(0) = 0, I′(0) = 0 → C₁ = 0, 3C₂ + 27/10 = 0 → C₂ = −9/10. I(t) = −(9/10)sin3t + (27t/10)cos3t A. Resonance confirmed."},
